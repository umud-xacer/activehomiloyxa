...
def _client_ip(request: Request) -> str:
    ...
    nodes/load balancing), which was silently defeating IP-scope lockout's whole point even after
    the `X-Real-IP` fix below (confirmed live: four failed attempts against one account from one
    real browser did not lock out a fifth, unrelated account from the "same" apparent IP).
    Cloudflare itself sets `CF-Connecting-IP` from the real TCP connection on every proxied
    request, which nginx passes through unmodified (no `proxy_set_header` overrides or hides it).

    Residual gap, not fully closed here: nginx's `server_name` also matches the bare EC2 IP
    (`16.171.149.72`), so direct non-Cloudflare access is possible today, and a request arriving
    that way COULD carry a spoofed `CF-Connecting-IP` header nothing here can distinguish from a
    real one. Not a full bypass of brute-force protection even so -- the account-scope lockout
    (keyed by email, independent of any IP header) still catches a password-grinding attacker
    regardless; only the IP-scope's precision is affected, and only via that specific
    direct-IP-access path. Properly closing it means either an AWS security-group rule
    restricting :80/:443 to Cloudflare's published IP ranges, or nginx's `ngx_http_realip_module`
    with `set_real_ip_from` scoped to those same ranges -- real infrastructure changes, out of
    scope for an application-code fix, flagged here rather than silently left unmentioned.

    `X-Real-IP` (see above) is the second fallback -- still correct for any future non-Cloudflare
    topology (a different CDN, or none) as long as nginx remains the sole ingress. `X-Forwarded-
    For`'s first hop is deliberately never used: nginx's `$proxy_add_x_forwarded_for` appends to
    whatever the client already sent, so an attacker-supplied first entry survives untouched.
    Falls back to the TCP peer for local dev (no nginx/Cloudflare in front) and any other topology
    where none of the above headers are present."""
