"""Fixtures for `media`'s real-Postgres integration tests (TEST-01: synthetic data only). Skips
gracefully when POSTGRES_HOST isn't set (local `scripts/test.sh` without `dev-up.sh` running);
runs for real in CI. Mirrors `apps/backend/tests/identity/integration/conftest.py`'s pattern
exactly."""
...
from media.infrastructure.persistence import models  # noqa: F401  registers all ORM classes
...
async def engine() -> AsyncIterator[AsyncEngine]:
    ...
    async with eng.begin() as conn:
        ...
        await conn.run_sync(MediaBase.metadata.create_all, checkfirst=True)
    yield eng
    await eng.dispose()


@pytest_asyncio.fixture(autouse=True)
async def _clean_media_tables(engine: AsyncEngine) -> AsyncIterator[None]:
    table_names = [t.name for t in MediaBase.metadata.tables.values()]
    async with engine.begin() as conn:
        qualified = ", ".join(f'"media"."{name}"' for name in table_names)
