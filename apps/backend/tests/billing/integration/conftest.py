"""Fixtures for `billing`'s real-Postgres integration tests (TEST-01: synthetic data only). Skips
...
exactly, plus `billing.invoice_number_seq` (`InvoiceRepository.next_invoice_number`'s own
backing sequence, created by this module's migration).
"""
...
async def engine() -> AsyncIterator[AsyncEngine]:
    ...
    async with eng.begin() as conn:
        ...
        await conn.run_sync(BillingBase.metadata.create_all, checkfirst=True)
        await conn.execute(text("CREATE SEQUENCE IF NOT EXISTS billing.invoice_number_seq"))
    yield eng
    await eng.dispose()


@pytest_asyncio.fixture(autouse=True)
async def _clean_billing_tables(engine: AsyncEngine) -> AsyncIterator[None]:
    table_names = [t.name for t in BillingBase.metadata.tables.values()]
    async with engine.begin() as conn:
        qualified = ", ".join(f'"billing"."{name}"' for name in table_names)
        await conn.execute(text(f"TRUNCATE TABLE {qualified} RESTART IDENTITY CASCADE"))
        await conn.execute(text("ALTER SEQUENCE billing.invoice_number_seq RESTART WITH 1"))
    yield
