"""Fixtures for `ads`' real-Postgres integration tests (TEST-01: synthetic data only). Skips
gracefully when POSTGRES_HOST isn't set (local `scripts/test.sh` without `dev-up.sh` running);
runs for real in CI. Mirrors `apps/backend/tests/notifications/integration/conftest.py`'s pattern
exactly.
"""
...
async def engine() -> AsyncIterator[AsyncEngine]:
    ...
    async with eng.begin() as conn:
        await conn.execute(text('CREATE SCHEMA IF NOT EXISTS "ads"'))
        await conn.run_sync(AdsBase.metadata.create_all, checkfirst=True)
    yield eng
    await eng.dispose()


@pytest_asyncio.fixture(autouse=True)
async def _clean_ads_tables(engine: AsyncEngine) -> AsyncIterator[None]:
    table_names = [t.name for t in AdsBase.metadata.tables.values()]
    async with engine.begin() as conn:
        qualified = ", ".join(f'"ads"."{name}"' for name in table_names)
