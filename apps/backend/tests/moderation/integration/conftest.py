"""Fixtures for `moderation`'s real-Postgres integration tests (TEST-01: synthetic data only).
Skips gracefully when POSTGRES_HOST isn't set (local `scripts/test.sh` without `dev-up.sh`
running); runs for real in CI. Mirrors `apps/backend/tests/profiles/integration/conftest.py`'s
pattern exactly.
"""
...
from moderation.infrastructure.persistence import models  # noqa: F401  registers all ORM classes
...
async def engine() -> AsyncIterator[AsyncEngine]:
    ...
    async with eng.begin() as conn:
        await conn.execute(text('CREATE SCHEMA IF NOT EXISTS "moderation"'))
        await conn.run_sync(ModerationBase.metadata.create_all, checkfirst=True)
    yield eng
    await eng.dispose()


@pytest_asyncio.fixture(autouse=True)
async def _clean_moderation_tables(engine: AsyncEngine) -> AsyncIterator[None]:
    table_names = [t.name for t in ModerationBase.metadata.tables.values()]
    async with engine.begin() as conn:
        qualified = ", ".join(f'"moderation"."{name}"' for name in table_names)
