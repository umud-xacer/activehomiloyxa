"""Fixtures for `profiles`' real-Postgres integration tests (TEST-01: synthetic data only). Skips
...
from profiles.infrastructure.persistence import models  # noqa: F401  registers all ORM classes
...
async def _clean_profiles_tables(engine: AsyncEngine) -> AsyncIterator[None]:
    table_names = [t.name for t in ProfilesBase.metadata.tables.values()]
    async with engine.begin() as conn:
        qualified = ", ".join(f'"profiles"."{name}"' for name in table_names)
