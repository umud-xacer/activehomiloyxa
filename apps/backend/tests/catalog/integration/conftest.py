"""Fixtures for `catalog`'s real-Postgres integration tests (TEST-01: synthetic data only). Skips
gracefully when POSTGRES_HOST isn't set (local `scripts/test.sh` without `dev-up.sh` running);
runs for real in CI. Mirrors `apps/backend/tests/media/integration/conftest.py`'s pattern exactly.
"""
...
from catalog.infrastructure.persistence import models  # noqa: F401  registers all ORM classes
...
async def _clean_catalog_tables(engine: AsyncEngine) -> AsyncIterator[None]:
    table_names = [t.name for t in CatalogBase.metadata.tables.values()]
    async with engine.begin() as conn:
        qualified = ", ".join(f'"catalog"."{name}"' for name in table_names)
