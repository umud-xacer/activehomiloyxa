...
function CategoryPill({ cat, index }: { cat: Cat; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative flex size-16 items-center justify-center overflow-hidden rounded-2xl border border-border bg-white p-2 shadow-soft transition-all duration-300 group-hover:-translate-y-0.5 group-hover:shadow-glow sm:size-20">
          {cat.image ? (
            <img src={cat.image} alt="" className="size-full rounded-xl object-contain" />
          ) : Icon ? (
