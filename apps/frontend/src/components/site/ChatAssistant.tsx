...
const WELCOME: ChatMessage = {
  id: "welcome",
  role: "assistant",
  text: "Assalomu alaykum! Men Active Home yordamchisiman. Uy va mulk, xizmatlar, investorlar, bron qilish yoki kompaniyalar haqida so'rashingiz mumkin.",
};

/**
 * Global "AI Chat Assistant" -- replaces the navbar's old "E'lon joylash" CTA (rendered here in
 * the same navbar, on every page, so it's always one tap away). The trigger is the pill button;
 * the panel is a `fixed`-positioned overlay so it isn't clipped by the navbar's own rounded/glass
 * pill. Answering logic lives in `features/assistant/knowledge-base.ts` -- deliberately
  ...
  return (
    <>
      <button
        ...
        className="group ml-1 inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-soft transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:scale-[1.03] hover:shadow-glow active:scale-[0.98]"
      >
        ...
        {open && (
          <>
            <motion.button
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[70] cursor-default bg-black/10 backdrop-blur-[2px] sm:bg-transparent sm:backdrop-blur-none"
            />
            <motion.div
              ...
              className="glass fixed right-4 top-[4.75rem] z-[71] flex h-[min(32rem,75vh)] w-[calc(100vw-2rem)] max-w-sm flex-col overflow-hidden rounded-3xl shadow-elevated sm:right-6"
            >
              {/* Header */}
